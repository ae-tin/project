...
class ECAPA_TDNN(nn.Module):
    ...
    def __init__(self, C):
        ...
        self.gaw = GraphAttentionLayer(200,200)
   #     self.cfw = Conv_Freq_Weight(80,100)
   #     self.sfm = Sigma_Freq_Masking(1)
   #     self.safw = Self_Att_Freq_weight(80)
   #     self.sabfw= Self_Att_Bundle_Freq_weight(80)
...
#            avgi = self.avgpool(x)
#            print('avg : ',avgi)
#            we = torch.add(maxi,avgi)
#            print('add : ',we)
#            we = NormalizeData2(we)+1e-2
#            print('normal : ',we)
            #we = torch.nan_to_num(we, nan=torch.mean(we[~we.isnan()]))   
#            we = we.mean(dim=0,keepdim=True)   
#            print('mean:',we)
#            we = NormalizeData3(we) + 1e-2
#            print('norm:',we)
                   # 배치에 대한 freq 요약값 (1,80,1)
    #        we = self.sfi(x)
