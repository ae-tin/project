...
def feature_extract_raw_melspec(fnm, rand = False, samp_sec = 20, sr = 4000, pre_emphasis = 0, hop_length=256, win_length = 512, n_mels = 120):
...
def feature_extract_tsv_raw(fnm,tsv,rand = False, samp_sec = 2, sr = 4000, pre_emphasis = 0):
...
def feature_extract_tsv_raw_one(fnm,tsv,rand = False, samp_sec = 2, sr = 4000, pre_emphasis = 0):
...
#            filename_location = recording_file.split('_')[1].split('.')[0]
            ...
            raw1, mur_samp = feature_extract_raw(filename,samp_sec=30)
            features['raw'].extend(raw1)
            mel = feature_extract_raw_melspec(filename)  
            out_samp = len(mel)
            features['mel'].extend(mel)
            ...
            current_murmur = np.repeat(current_murmur,mur_samp,0)
            ...
            current_outcome = np.repeat(current_outcome,out_samp,0)         ############## out_samp
