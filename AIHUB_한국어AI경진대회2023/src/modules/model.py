...
class DeepSpeech2(EncoderModel):
    ...
    def __init__(
        ...
        self.masking = SequenceMaskLayer()
 ...
 #       print('input_lengths : ',input_lengths)
        ...
        cnn_outputs = outputs.permute(1, 0, 2).contiguous()
        masked_cnn_outputs = self.masking(cnn_outputs)

#        print('CNN outputs shape: ',outputs.shape)
        for i, rnn_layer in enumerate(self.rnn_layers):
            if i == 0 :
                mid_outputs = rnn_layer(cnn_outputs, output_lengths)
            elif i == len(self.rnn_layers)-1 :
                rnn_outputs = rnn_layer(mid_outputs, output_lengths)
            else:
                mid_outputs = rnn_layer(mid_outputs, output_lengths)
#        print('RNN outputs shape: ',outputs.shape)
        for i, rnn_layer in enumerate(self.rnn_layers):
            if i == 0 :
                mid_outputs = rnn_layer(masked_cnn_outputs, output_lengths)
            elif i == len(self.rnn_layers)-1 :
                masked_rnn_outputs = rnn_layer(mid_outputs, output_lengths)
            else:
                mid_outputs = rnn_layer(mid_outputs, output_lengths)

        outputs = self.fc(rnn_outputs.transpose(0, 1)).log_softmax(dim=-1)
        
        
        return outputs, output_lengths, rnn_outputs, masked_rnn_outputs
...
class SequenceMaskLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.ratio > 0:
            is_valid = x != 0
            is_valid = is_valid * 1
            is_valid = is_valid.to(x.get_device())
...
def build_model(
    ...
    if config.self_pretrained :
        ...
        model.load_parameters(config.self_pretrained_model_path)
        
        model = nn.DataParallel(model)
        print('*************************************************************************************')
        print('****************** {} pretrained with KsponSpeech Dataset Loaded. ******************'.format(config.self_pretrained_model_path))
        print('*************************************************************************************')
        
    elif config.kspon_pretrained :
        print(os.getcwd())
        # https://github.com/kthworks/KoreanSTT-DeepSpeech2/tree/main
        model_kspon = torch.load(os.path.join(os.getcwd(), 'model_ds2.pt'), map_location=lambda storage, loc: storage).to(device)
        if isinstance(model_kspon, nn.DataParallel):
            model_kspon = model_kspon.module
        ...
        loaded_state = model_kspon.state_dict()
        for name, param in loaded_state.items():
            origname = name
            if name not in self_state:
                name = name.replace("module.", "")
        ...
        model = nn.DataParallel(model)
        print('*************************************************************************************')
        print('***************** Model pretrained with KsponSpeech Dataset Loaded. *****************')
        print('*************************************************************************************')
#        print(model)
#        print('*************************************************************************************')
    else :
