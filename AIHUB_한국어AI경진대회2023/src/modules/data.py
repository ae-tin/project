...
class SpectrogramDataset(Dataset, SpectrogramParser):
    ...
    def __init__(
        ...
        super(SpectrogramDataset, self).__init__(
            ...
            audio_extension=audio_extension, rir_path=config.rir_path, musan_path=config.musan_path, aug = config.noise_aug,mode = mode
        )
    ...
    def __getitem__(self, idx):
        ...
        ## 고친거
# kspon        feature = self.parse_audio(self.audio_paths[idx], self.augment_methods[idx])
# 교육상담        feature = self.parse_audio(os.path.join(self.dataset_path, self.audio_paths[idx].split('_')[0], self.audio_paths[idx].split('_')[1]+'.wav'), self.augment_methods[idx])
...
def split_dataset(config, transcripts_path: str, vocab: Vocabulary, valid_size=.03):
    """
    ...
    train_audio_paths, test_audio_paths, train_transcripts, test_transcripts = train_test_split(audio_paths,
                                                                                                  transcripts,
...
def test_dataset(config, transcripts_path: str, vocab: Vocabulary, valid_size=.2):
    """
    split into training set and validation set.
