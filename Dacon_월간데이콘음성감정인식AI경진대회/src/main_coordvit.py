...
def main(args: argparse.Namespace) -> None:
    ...
    model_tag_o = "emotion_{}_ep{}_bs{}_lr{}_w2v2_TL{}_frz{}_{}_aug|{}".format(
        os.path.splitext(os.path.basename(args.config))[0],
        config["num_epochs"], config["batch_size"],config["optim_config"]["base_lr"]
        ,config["model_config"]["total_transformer_layers"],config["model_config"]["n_frz_layers"],classifier,aug)
    if args.comment:
...
def get_model(model_config: Dict, device: torch.device):
    ...
    _model = getattr(module, "CoordVIT")
