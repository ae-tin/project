...
def main(args: argparse.Namespace) -> None:
    ...
    model_tag_o = "emotion_{}_ep{}_bs{}_lr{}_w2v2_TL{}_frz{}_{}_sec{}_spec{}_noi{}".format(
        os.path.splitext(os.path.basename(args.config))[0],
        config["num_epochs"], config["batch_size"],config["optim_config"]["base_lr"]
        ,config["model_config"]["total_transformer_layers"],config["model_config"]["n_frz_layers"],classifier,audio_sec,aug,aug2)
    if args.comment:
...
def get_loader(
    ...
    train_set = Dataset_Emotion_TrainDev(file_list=file_train, labels=d_label_trn,config=config,mode='Train')
    ...
    train_set_embed = Dataset_Emotion_TrainDev(file_list=file_train, labels=d_label_trn,config=config,mode='Train',ext_embed=True)
    ...
    dev_set = Dataset_Emotion_TrainDev(file_list=file_dev, labels=d_label_dev,config=config)
    ...
    dev_set_embed = Dataset_Emotion_TrainDev(file_list=file_dev, labels=d_label_dev,config=config,ext_embed=True)
    ...
    eval_set = Dataset_Emotion_Eval(file_list=file_eval,config=config)
...
def produce_train_embedding_file(
    ...
    if mode == 'train':
        rate = '95'
        submission = pd.read_csv('./data/{}_{}_aug.csv'.format(mode,rate))
    else :
        rate = '05'
        submission = pd.read_csv('./data/{}_{}.csv'.format(mode,rate))
    check_id = submission.id
    embedding_dict = {}
    embedding_list = torch.empty(0)
    fname_list = []
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config",
                        ...
                        default="./config/wav2vec2_feat_hybridconv.conf")
                   #     required=True)
    parser.add_argument(
        "--output_dir",
        dest="output_dir",
