...
def main(args: argparse.Namespace) -> None:
    ...
    proj = model_config["projection"]
    print("projection_dim : ", proj)
    n_block = model_config["num_encoder_blocks"]
    print("num_encoder_blocks : ", n_block)
    ...
    model_tag_o = "emotion_{}_ep{}_bs{}_lr{}_w2v2_TL{}_frz{}_{}_proj{}_bl{}".format(
        os.path.splitext(os.path.basename(args.config))[0],
        config["num_epochs"], config["batch_size"],config["optim_config"]["base_lr"]
        ,config["model_config"]["total_transformer_layers"],config["model_config"]["n_frz_layers"],classifier,proj,n_block)
    if args.comment:
...
#    device = "cuda" if torch.cuda.is_available() else "cpu"
    ...
    # define dataloaders
    trn_loader, trn_loader_embed, dev_loader, eval_loader = get_loader(
...
def get_model(model_config: Dict, device: torch.device):
    ...
    _model = getattr(module, "Wav2Vec2_CoordVIT_Model")
...
def get_loader(
    ...
    trn_list_path = pd.read_csv('./data/train_95.csv')
    ...
    return trn_loader, trn_loader_embed, dev_loader, eval_loader
...
def produce_evaluation_file2(
    ...
    fnm = save_path.replace('.csv','_test.pk')
...
def produce_train_embedding_file(
    ...
    for fn, test_id, embedding in zip(fname_list, check_id,embedding_list):
        assert fn == test_id
        embedding_dict[test_id] = embedding
    fnm = save_path.replace('.csv','_train.pk')
    with open(fnm, 'wb') as f:
        pk.dump(embedding_dict, f)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config",
                        ...
                        default="./config/wav2vec2_coordvit.conf")
                   #     required=True)
    parser.add_argument(
        "--output_dir",
        dest="output_dir",
