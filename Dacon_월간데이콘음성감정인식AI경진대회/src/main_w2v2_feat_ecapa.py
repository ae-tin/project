...
def main(args: argparse.Namespace) -> None:
    ...
    model_tag_o = "emotion_{}_ep{}_bs{}_lr{}_w2v2_TL{}_frz{}_{}_sec{}".format(
        os.path.splitext(os.path.basename(args.config))[0],
        config["num_epochs"], config["batch_size"],config["optim_config"]["base_lr"]
        ,config["model_config"]["total_transformer_layers"],config["model_config"]["n_frz_layers"],classifier,sec)
    if args.comment:
...
def get_model(model_config: Dict, device: torch.device):
    ...
    _model = getattr(module, "Wav2Vec2_feat_ECAPA_Model")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config",
                        ...
                        default="./config/wav2vec2_feat_ecapa.conf")
                   #     required=True)
    parser.add_argument(
        "--output_dir",
        dest="output_dir",
