...
class AutosomePredictor(Predictor):
    ...
    def _add_channels(self, seq: torch.Tensor, rev_value: int):
        ...
        if self.use_single_channel:
            single = torch.full( (0, self.seqsize) , 0, dtype=torch.float32)
    ...
    def predict(self, sequence: str) -> float:
        ...
        x_rev = self._add_channels(
            self._preprocess_sequence(self._add_plasmid(revcomp(sequence))),
            rev_value=1
        )
        x = x.to(self.device)
        y = self.model(x)[-1].detach().cpu().flatten().tolist()
        
        x = x.to(self.device)
        y_rev = self.model(x_rev)[-1].detach().cpu().flatten().tolist()
