...
parser.add_argument('--save_path',  type=str,   default="exps/first_classification/final_lightgbm_grid_score.txt", help='Path to save the score.txt and models')
...
#    params = {'max_depth' : np.random.choice([3,5,6,10,12]),
#              'subsample' : np.random.choice([.3,.5,.8, 1]),
#             'colsample_bytree' :np.random.choice([.3,.5,.8, 1]),        # tree에 depth
#             'n_estimators': np.random.choice([10,50,100,300,500])
#             }
    params = {'num_leaves': np.random.choice([10,30,50,70]),
             'early_stopping_round' : 30,
             'lambda': np.random.choice([0.1,0.3,0.5]),
             'max_depth' : np.random.choice([3,5,6,10,12]),
    #         'boosting' : np.random.choice(['gdbt','dart','goss']),  #'rf',
             'num_boost_round' : np.random.choice([50,100,300,500,700]),
             'learning_rate' : np.random.choice([0.001,0.005,0.01,0.05,0.1,0.3]),
             'objective': 'binary', 
