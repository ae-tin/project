...
cat_col = ['purpose','income_type','employment_type','houseown_type']
...
for j in tqdm(range(200)):
    ...
    datatype = np.random.choice(['label_encoding'])

#    params = {'learning_rate': np.random.choice([0.01, 0.03, 0.05, 0.1, 0.3]),
#          'depth': np.random.choice([4, 5, 6, 8, 10]),
...
#          'iterations': np.random.choice([10,50,100,200,300,500,700,1000]),
   #       'class_weights' : [0.1, 4]
#         }
    params = {'iterations': np.random.choice([10,50,100,200,300,500,700,1000]),
          'depth': np.random.choice([4, 5, 6, 8, 10]),
          'loss_function': 'CrossEntropy',
          'leaf_estimation_iterations': np.random.choice([10,30,50]),
           'eval_metric': 'F1',
    ...
    for i in range(2):
        ...
        start = time.time()
        clf.fit(X_train, y_train,cat_features = cat_idxs, eval_set = (X_valid,y_valid), early_stopping_rounds = 30)
